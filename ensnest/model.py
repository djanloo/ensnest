...
class Model:
    ...
    def _check(self):
        ...
        if (
            not isinstance(log_prior_result, np.ndarray)
            or log_prior_result.shape != testshape
        ):
            raise ValueError(
        ...
        if (
            not isinstance(self.log_likelihood(dummy1), np.ndarray)
            or self.log_likelihood(dummy1).shape != testshape
        ):
        ...
        if not (result1 == result2).any():
            ...
            raise ValueError(
                "Bad-behaving log_likelihood:"
                " different results for different iteration order" + compare
            )
        ...
        self.log_likelihood_execution_time_estimate = (end - start) / N_TIMES_EVAL
    ...
    def is_inside_bounds(self, points):
        ...
        is_coordinate_inside = np.logical_and(
            points > self.bounds[0], points < self.bounds[1]
        )
    ...
    def __hash__(self):
        ...
        points = (
            np.linspace(0, 1, 10)[:, None] * (self.bounds[1] - self.bounds[0])
            + self.bounds[0]
        )
...
class Rosenbrock(Model):
    ...
    def log_likelihood(self, var):
        return (
            -0.5 * (var["y"] - var["x"] ** 2) ** 2 - 1.0 / 20.0 * (1.0 - var["x"]) ** 2
        )
...
class UniformSphere(Model):
    ...
    def __init__(self, R=10.0, dim=1.0):
        self.dim = dim
        self.R = R
        super().__init__()
