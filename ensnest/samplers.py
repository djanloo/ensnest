...
class Sampler:
    ...
    def __init__(self, model, mcmc_length, nwalkers, verbosity=0):
        ...
        # methods reduce to normal definition.
        self.chain = np.zeros((self.length, self.nwalkers),
                              dtype=self.model.livepoint_t).squeeze()

        # uniform initialisation
        for walker in range(self.nwalkers):

            self.chain['position'][0, walker] = U(*self.model.bounds)
            self.chain['logP'][0, walker] = self.model.log_prior(
                self.chain[0, walker]['position'])
            self.chain['logL'][0, walker] = self.model.log_likelihood(
                self.chain[0, walker]['position'])
...
class AIESampler(Sampler):
    ...
    def AIEStep(self, Lthreshold=None, continuous=False):
        ...
        # if a threshold Lmin is set, sets as 'impossible' the proposals
        # outside
        if Lthreshold is not None:
            log_prior_proposal[log_likelihood_proposal < Lthreshold] = -np.inf

        log_accept_prob = (self.model.space_dim - 1) * \
            np.log(z) + log_prior_proposal - log_prior_current
    ...
    def reset(self, start=None):
        ...
        self.chain = np.ones((self.length, self.nwalkers),
                             dtype=self.model.livepoint_t).squeeze()
        if start is not None:
            assert isinstance(
                start, np.ndarray), 'Chain start point is not a np.ndarray'
            assert start.shape == (
                self.nwalkers,), 'Chain start point has wrong shape'
    ...
    def set_length(self, length):
        ...
        self.length = length
        new_chain = np.zeros((self.length, self.nwalkers),
                             dtype=self.model.livepoint_t).squeeze()
        new_chain[:min(old_length, length)
                  ] = self.chain[:min(old_length, length)]
...
class AIEevolver(AIESampler):
    ...
    def get_new(
        ...
        for t in tqdm(
                ...
                leave=False,
                desc='evolving',
                bar_format=BAR_FMT_EVOL,
                disable=not progress):
            self.AIEStep(Lthreshold=Lmin, continuous=True)

        # this part requires a time-expensive check each loop
        # but since it is permanent it will allegedly be performed once or
        # twice
        while True:
            # counts the duplicates
            is_duplicate = (self.chain['logL'][self.elapsed_time_index] ==
                            self.start_ensemble['logL'][:, None]).any(axis=0)  # time comsuming op
            ...
            if n_duplicate == 0:
                ...
                if not allow_resize:
                    print(
                        'CRITICAL: ensemble did not evolve due to too low number of steps (resize unallowed).')
                    exit()
                print(
    ...
    def bring_over_threshold(self, logLthreshold):
        ...
        with tqdm(total=1, desc='bringing over threshold') as pbar:
            while logLmin < logLthreshold:
                sorted = np.sort(
                    self.chain[self.elapsed_time_index], order='logL')
