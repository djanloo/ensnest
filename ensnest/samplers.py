...
class Sampler:
    ...
    def __init__(self, model, mcmc_length, nwalkers):
        ...
        self.elapsed_time_index = (
            0  # 'time' index up to which self.chain has been developed
        )

        # the squeeze method is for mantaining generality. Single-walker
        # methods reduce to normal definition.
        self.chain = np.zeros(
            (self.length, self.nwalkers), dtype=self.model.livepoint_t
        ).squeeze()
...
class AIESampler(Sampler):
    ...
    def __init__(self, model, mcmc_length, nwalkers=10, a=None):

        super().__init__(model, mcmc_length, nwalkers)
        self.a = 3.0 if a is None else a
        if self.a <= 1:
    ...
    def get_stretch(self, size=1):
        ...
        """
        return (
            U(0, 1, size=size) * (self.a ** (1 / 2) - self.a ** (-1 / 2))
            + self.a ** (-1 / 2)
        ) ** 2
    ...
    def reset(self, start=None):
        ...
        if start is not None:
            assert isinstance(
                start, np.ndarray
            ), "Chain start point is not a np.ndarray"
...
class AIEevolver(AIESampler):
    ...
    def __init__(self, model, steps, length=None, nwalkers=10, a=None):

        # if lenght is not specified, runs continuously over two times
        if length is None:
            length = 2

        super().__init__(model, length, nwalkers=nwalkers, a=a)
        self.steps = steps
    ...
    def init(self, progress_position=0, init_positions=None):
        if init_positions is not None:
            ...
            self.chain["logP"][0] = self.model.log_prior(self.chain[0]["position"])
            self.chain["logL"][0] = self.model.log_likelihood(self.chain[0]["position"])
    ...
    def get_new(self, Lmin, start_ensemble=None, progress=False, allow_resize=True):
        ...
        while True:
            ...
            is_duplicate = (
                self.chain["logL"][self.elapsed_time_index]
                == self.start_ensemble["logL"][:, None]
            ).any(
                axis=0
            )  # time comsuming op
